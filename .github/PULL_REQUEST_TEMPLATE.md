...
> This template is for PRs that include changes to `package-lock.json` / `package.json`.
...
- [ ] I ran `npm install` to regenerate the lock file
...
- [ ] `npm audit` run and clean after adding
...
- [ ] No new CVEs introduced (checked via `npm audit`)
...
```
<!-- Paste the output of `npm audit` here -->
```

- [ ] `npm audit` shows no new vulnerabilities introduced by this PR
...
- [ ] No known CVEs introduced (verified via `npm audit` output above)
...
- [ ] _(optional)_ I ran `npm ci` locally and the build passes
